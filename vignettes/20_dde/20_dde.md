...
> [!IMPORTANT]
>
> ### Solvers that do not support DDEs
>
> Not every solver accepts a delay problem, and two of the failures are
> noisy rather than graceful:
>
...
| **Compatible PSM solvers** | All | LAML, GCVSolver, AdamSolver, VariationalSolver |
...
provides delayed state values. Four solvers work with DDEs via automatic
wrapping with `MethodOfSteps`: `LAML`, `GCVSolver`, `AdamSolver` and
`VariationalSolver`.
