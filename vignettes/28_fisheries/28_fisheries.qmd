...
LAML with Poisson likelihood. The Poisson likelihood uses a log link,
so the loss is $\sum_i (\hat\mu_i - y_i \log \hat\mu_i)$.
...
fit. With `method=:parametric` and a Poisson likelihood, each bootstrap
replicate samples new counts from `Poisson(μ̂_t)` and refits the model.
