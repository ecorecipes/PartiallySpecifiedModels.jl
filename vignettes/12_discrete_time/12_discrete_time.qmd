...
shooting refinement) both recover the function accurately.
...
dynamics have a longer transient, giving both solvers more information across
...
`BNGSolver`, `AdamSolver`, `MultipleShootingSolver`, `DerivativeFreeSolver`,
`MCMCSolver`, `VariationalSolver`, `ABCSolver`, `EnsembleKalmanSolver` and
...
(`n_samples=0`, the default). Setting `n_samples > 0` selects the tempered
population-MCMC path, which errors on a discrete problem.
