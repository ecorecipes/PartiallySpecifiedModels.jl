...
sol_coll = solve(prob_coll, CollocationLAML(maxiters=30, verbose=false,
    lambda_ode_start=1.0, lambda_ode_end=1e4, n_continuation=6))
...
uf_bh = BSplineApproximator(:f, (0.0, 600.0), 10;
                              initial=N -> 2.0 * N / (1.0 + N / 600.0))

prob_bh = PSMProblem(bh_psm!, N0_bh, tspan_bh, [uf_bh];
    data_times=times_bh,
    data_values=reshape(data_bh, :, 1),
    obs_to_state=[1], known_params=NamedTuple(),
    discrete=true)

sol_adam = solve(prob_bh, AdamSolver(maxiters=3000, lr=0.01, verbose=false))
...
```{julia}
#| echo: false
sol_bh_laml = solve(prob_bh, LAML(maxiters=200, verbose=false))
...
A standard 4-panel diagnostic display assesses residual behaviour for the Ricker LAML fit. The QQ plot checks normality of standardized residuals, "Residuals vs Fitted" detects systematic patterns, the histogram visualises the residual distribution, and "Observed vs Fitted" checks overall calibration.
...
- **CollocationLAML** — Less effective for discrete maps than for continuous ODEs. The
  collocation treats each state value as a free parameter, so the map compliance penalty
  `λ_ode × Σ(u_{t+1} - f(u_t))²` competes with the data fit. For short time series where
  the number of state parameters equals the number of observations, collocation is effectively
  unconstrained and tends to overfit (high EDF).
