...
The plot is restricted to the range of $I/N$ actually observed in the
data. Outside this range the spline is unconstrained by data and
controlled only by the smoothing penalty, so extrapolation is unreliable
and can diverge significantly — especially for families (Gaussian,
Truncated Normal) whose profiled scale parameter produces different
smoothing from count-based families.

``` julia
# Restrict grid to the observed data range
Imin_obs = minimum(I_true) / N
Imax_obs = maximum(I_true) / N
I_grid = range(Imin_obs, Imax_obs, length=200)
...
> [!NOTE]
>
> ### Why does smoothing differ across likelihoods?
>
> Different families produce different amounts of smoothing for three
> reasons:
>
> 1.  **Different data**: Each family sees data generated by a different
>     noise process (Gaussian noise, Poisson counts, etc.), so even with
>     an identical smoother the optimal fit differs.
> 2.  **Different effective scale**: The Fellner–Schall update computes
>     $\lambda_{\text{new}} = \hat\phi \times \text{edf} / (\beta' S \beta)$,
>     where $\hat\phi$ is the effective dispersion. For Gaussian,
>     $\hat\phi = \hat\sigma^2$ (profiled, often $\gg 1$); for Poisson,
>     $\hat\phi$ is the Pearson dispersion (typically $\approx 1$). This
>     scale difference directly affects the smoothing parameter.
> 3.  **Different IRLS weights**: Poisson weights ($w / \mu$)
>     down-weight high-count observations relative to Gaussian (unit
>     weights), changing which parts of the curve are fitted most
>     tightly.
...
the Gaussian fit, which serves as the baseline likelihood.
